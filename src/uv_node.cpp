...
/*
void UAVNode::define_subject(uint16_t subject_id, PGM_P dtf_name, UAVPortFunction fn) {
    UAVNodePortInfo* port_info = ports.port_claim(subject_id, dtf_name);
    if(port_info==nullptr) return;
    if(fn==nullptr) {
        port_info->is_output = true;
...
void UAVNode::transport_add(UAVTransport *transport) {
    _transports.push_back(transport);
    transport->start(*this);
}

void UAVNode::transport_remove(UAVTransport *transport) {
...
void UAVNode::subscribe(UAVNodeID remote_node_id, UAVPortID subject_id, UAVPortListener fn) { 
    auto key = std::make_tuple(remote_node_id, subject_id);
    _subscribe_nodeport[key] = fn;
}

void UAVNode::subscribe(UAVPortID subject_id, UAVPortListener fn) { 
